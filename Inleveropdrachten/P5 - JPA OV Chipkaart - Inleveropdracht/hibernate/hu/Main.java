...
import hibernate.dao.OVChipkaartService;
import hibernate.dao.ReizigerService;
import hibernate.hu.ConsoleColor;
import hibernate.hu.HibernateUtil;
import hibernate.hu.OVChipkaart;
	...
	public static void main(String[] args) {
		...
		if (true) {
			...
			reiziger = new Reiziger(6, "Stefan", "van", "Echtelt",
					Date.valueOf(new GregorianCalendar(2000, Calendar.FEBRUARY, 10).toInstant()
							.atZone(ZoneId.systemDefault()).toLocalDate()));

			ovChipkaart = new OVChipkaart(12345, Date.valueOf(new GregorianCalendar(2019, Calendar.DECEMBER, 10)
					.toInstant().atZone(ZoneId.systemDefault()).toLocalDate()), 2, 50f, reiziger);
